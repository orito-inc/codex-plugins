...
- 目的・完了条件、確定した範囲と制約、必要な未決事項を簡潔にまとめ、実装方針・影響箇所・検証方法は確定情報がある場合だけ含めます。
...
- 何であるか・なぜ存在するか、具体例または有用なたとえ、実際の用語との対応、3〜5要素の図、重要な限界・境界の順で説明します。
...
`minimalist`は、文書やコードを要件・目的・前提から見直し、合意された成果を満たす最小成立形へ削減するSkillです。
...
レビュー・編集する対象とともに明示的に呼び出します。

```text
$minimalist を使って、この設計書を目的・前提から見直し、最小成立形を提案してください。
```

### Behavior

- 利用者が得たい成果と受け入れ条件を基準に、目的・前提・要件・仕様、構造、実現方法の順で削減を検討します。
- 明示された必須要件も検討対象にしますが、変更で失う成果や影響を示し、合意前には反映しません。
- 正しさ、必要な安全性・互換性、レイヤー依存規約を維持します。
- レビュー依頼では最小成立形と削除・統合案を返し、編集依頼では依頼範囲内で反映・検証します。
- 自動呼び出しは無効です。`$minimalist`で明示的に呼び出してください。

## Conformist

`conformist`は、リポジトリの類似実装を探し、既存パターン・慣習に合わせて設計やコードを作成・レビューするSkillです。
...
実装・レビューする対象とともに明示的に呼び出します。

```text
$conformist を使って、既存の類似実装と部品を活用してこの変更を実装してください。
```

### Behavior

- 同一機能・ドメイン、同一ディレクトリ・モジュール、リポジトリ内の同種機能の順に参考実装を探します。
- 明示ルールをすべての候補に適用し、現在の要件と依存規約に合う既存部品を再利用します。
- バグやセキュリティ問題、規約違反は引き継がず、必要な差分だけを設けます。
- レビュー依頼では対象・根拠・最小修正案を示し、編集依頼では対象範囲に反映してレイヤー依存と振る舞いを検証します。
- 自動呼び出しは無効です。`$conformist`で明示的に呼び出してください。
